...
//        System.out.println("File full path: "+nonPDRFile.getAbsolutePath());
    ...
    public static void writeNonPDRData(File nonPDRFile, String data){
        try
        {
            FileWriter writer = new FileWriter(nonPDRFile,true);
            writer.append(data);
            writer.flush();
            writer.close();
        }
    ...
    public static CommPortIdentifier initialize(String txtComPortName) {
        ...
        if (portId == null) {
            JOptionPane.showMessageDialog(null," Serial port is not availeble ","Please connect your device ",JOptionPane.ERROR_MESSAGE);
